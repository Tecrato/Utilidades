...
class Text_return(Base_win):
    def __init__(self, centro, encabezado, texto, large=False, size = (500,300)) -> None:
        ...
        if large:
            ...
            self.input = Input_text((size[0]-(150*2.25),150),(20,150), None, border_top_left_radius=20, border_bottom_left_radius=20, max_letter=40)

        self.input.draw(self.surface)

        self.botones.append({
            'btn':Create_boton('Cancelar',30,None,Vector2(size)-(20,20), 20, 'bottomright','black','white', border_width=-1),
            'return':'cancelar',
            'result': lambda:'cancelar'
        })
        self.botones.append({
            'btn':Create_boton('Aceptar',30,None,(Vector2(size)-(20,20))-(self.botones[1]['btn'].rect.w+20,0), 20, 'bottomright','black','white', border_width=-1),
            'return':'aceptar',
