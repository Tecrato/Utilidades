...
class Download:
    ...
    def prepare(self):
        ...
        try:
            response = self.session.head(self.url, allow_redirects=True, stream=True, timeout=30)
            if self.file == None:
                self.file: str = urlparse(self.url).path
                self.file: str = self.file.split('/')[-1]
                self.file: str = self.file.split('?')[0]
    ...
    def progress(self, value):
        ...
        if self.progress_func is not None:
            self.progress_func(self.__progress/self.size * 100)

        
        
class DownloadThread(Thread):
    def __init__(self, downloader, start, end, index):
    ...
    def download(self):
        ...
        try:
            ...
            with open(self.downloader.file, "+rb") as file:
                file.seek(self.inicio+self.progress)
